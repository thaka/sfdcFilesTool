...
//  ------------------------------------------------------------------------------------------------
	...
	* @Return:	
	*/ 
	public static void createSfdcRecords(PartnerConnection connectionTarget, ArrayList<SObject> listRecords) {
		// Create/Update the records
		try {
			int nrBatches = listRecords.size() / 200 + 1;
			for(int x = 0; x < nrBatches; x = x + 1) {
				int fromIndex = 200 * x;
				int toIndex = 200 * (x+1) < listRecords.size() ? 200 * (x+1) : listRecords.size();
